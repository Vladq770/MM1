...
def build_chart(root, time, step_time, scheme):
    ...
    for i in rows:
        temp = []
        temp.append(float(i.entry_x.get()))
        temp.append(float(i.entry_y.get()))
        temp.append(float(i.entry_vx.get()))
        temp.append(float(i.entry_vy.get()))
        temp.append(float(i.entry_L.get()))
        temp.append(float(i.entry_S.get()))
...
label6 = Label(table_labels, text='L', width=10, font=('Times 14'))
label6.pack(side=LEFT)
label7 = Label(table_labels, text='S', width=10, font=('Times 14'))
